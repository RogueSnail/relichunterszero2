...
if (vertical_speed_start < interactive_speed) && (instance_exists(other)) &&  (isInteractive)
{
    ...
    if (ammoType == type_light) if (global.ammo_light[p] < global.ammo_light_max) { global.ammo_light[p] += myAmmo; } else isAmmoFull = true;
    if (ammoType == type_medium) if (global.ammo_medium[p] < global.ammo_medium_max) { global.ammo_medium[p] += myAmmo; } else isAmmoFull = true;
    if (ammoType == type_heavy) if (global.ammo_heavy[p] < global.ammo_heavy_max) { global.ammo_heavy[p] += myAmmo; } else isAmmoFull = true;
    
    if (isAmmoFull)
    {
        guiInfo = instance_create(other.x,other.y,gui_info);
